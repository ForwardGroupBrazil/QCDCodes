TLegend * note_glb_unbinned_figures()
{
  //----------------------------------------------------
  // General macro setup
  //----------------------------------------------------
  ...
  for (int iLevel=0;iLevel < 1; ++iLevel) {
    for (int iQuantity=0;iQuantity < 1; ++iQuantity) {
      for (int iXaxis=0;iXaxis < 3; ++iXaxis) {
	e_canvas = newCanvas(figDirName2_+"/efficiencies/"+e_quantity[iQuantity]+e_xAxis2[iXaxis]+"_0_500",e_levelName[iLevel]+" "+e_histo[0]+" "+e_quantity[iQuantity]+e_xAxis2[iXaxis]);
  ...
  //-------------------------------
  TString dirNameRM_("RecoMuon_TrackAssoc/Trk");
  TString dirNameRM_2("RecoMuon_TrackAssoc/Sta");
  TString dirNameRM_3("RecoMuon_TrackAssoc/Glb");
  ...
  for (int iLevel=0;iLevel < 1; ++iLevel) {
	...
	erm_canvas = newCanvas(figDirName2_+"/efficiencies/"+erm_quantity[iQuantity]+erm_xAxis2[iQuantity]+"_0_500",erm_levelName[iLevel]+" "+erm_histo[iQuantity]+" "+erm_quantity[iQuantity]+erm_xAxis2[iQuantity]);
