TLegend * note_trkMu_figures()
{
  ...
  TString dirName_("MultiTrack/good_TrackerOnly_tpToTkmutkAssociation");
  TString figDirName_("trackerMuons/tkmu");
  ...
  TList * dirListRM = makeDirectoryCollection(fileList,directoriesRM,1);
  ...
  for (int iLevel=0;iLevel < 1; ++iLevel) {
	...
	erm_canvas = newCanvas(figDirName2_+"/efficiencies/"+erm_level[iLevel]+"_"+erm_quantity[iQuantity]+erm_xAxis2[iQuantity],erm_levelName[iLevel]+" "+erm_histo[iQuantity]+" "+erm_quantity[iQuantity]+erm_xAxis2[iQuantity]);
