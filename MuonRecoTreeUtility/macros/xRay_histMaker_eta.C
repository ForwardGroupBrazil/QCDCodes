...
int ScanTreeEta ( TTree* tree, char *fileName, bool isData=false,double weight = 1.0) {
  ...
  double ptLowerLimit = -3.;
  double ptUpperLimit = 3.;
  int nBins = 60;
  ...
  //  l3EtaRate->GetXaxis()->SetTitle("#eta");
  ...
  //  l2EtaRate->GetXaxis()->SetTitle("#eta");
  ...
  //  tkTrackEtaRate->GetXaxis()->SetTitle("#eta");
  ...
  for( int iEntry = 0; iEntry < nEntries; iEntry++) {
	...
	if(!isData) {
	  ...
	  sprintf(histoName,"l3EtaRate_%d",GetBinNum(999));	  
	  muonPtHistoMap[histoName]->Fill(pt_L3,weight);	  
	  //muonPtHistoMap["l3EtaRate_9"]->Fill(pt_L3,weight);
	}
      }
	...
	if(!isData) {
	  ...
	  char histoName[20];
	  //if(myL3ParentIdBin==Bins.size()) myL3ParentIdBin--;
	  sprintf(histoName,"tkTrackEtaRate_%d",GetBinNum(999));	  
  ...
  if(!isData){
    ...
    char histoName[20];
    sprintf(histoName,"l2EtaRate_%d",GetBinNum(999));
    l2EtaRate->Add(l2PtHistoMap[histoName]);
  }
