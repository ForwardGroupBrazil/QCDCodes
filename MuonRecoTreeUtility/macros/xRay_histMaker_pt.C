...
int ScanTreePt ( TTree* tree, char *fileName, bool isData=false,double weight = 1.0) {
  ...
  for (int i = 0; i != Bins.size(); ++i) {    
    ...
    tmpTh1->GetXaxis()->SetTitle("p_{T} [GeV]");
  ...
  tmpTh2->GetXaxis()->SetTitle("p_{T} [GeV]");
  ...
  tmpTh3->GetXaxis()->SetTitle("p_{T} [GeV]");
  ...
  //  l3PtRate->GetXaxis()->SetTitle("p_{T} [GeV]");
  ...
  //  l2PtRate->GetXaxis()->SetTitle("p_{T} [GeV]");
  ...
  //  tkTrackPtRate->GetXaxis()->SetTitle("p_{T} [GeV]");
  ...
  for( int iEntry = 0; iEntry < nEntries; iEntry++) {
	...
	if(!isData) {
	  ...
	  sprintf(histoName,"l3PtRate_%d",GetBinNum(999));	  
	  muonPtHistoMap[histoName]->Fill(pt_L3,weight);	  
	  //muonPtHistoMap["l3PtRate_9"]->Fill(pt_L3,weight);
	}
      }
	...
	if(!isData) {
	  ...
	  char histoName[20];
	  //if(myL3ParentIdBin==Bins.size()) myL3ParentIdBin--;
	  sprintf(histoName,"tkTrackPtRate_%d",GetBinNum(999));	  
  ...
  if(!isData){
    ...
    char histoName[20];
    sprintf(histoName,"l2PtRate_%d",GetBinNum(999));
    l2PtRate->Add(l2PtHistoMap[histoName]);
  }
