...
void test3()
{
  ...
  while( (h=(TH1*)iterG()) ) {
    ...
    //cout << pt[i] << " " << h->GetBinContent(bin) << " \\pm " << h->GetBinError(bin) << endl;
    //cout << pt[i] << " binSta " << binSta << " " <<hSta->GetBinContent(binSta) << " \\pm " << hSta->GetBinError(binSta) << endl;
