...
GlobalTruncRefitter::GlobalTruncRefitter(const edm::ParameterSet& par,
				       const MuonServiceProxy* service) : 
  theService(service) {

  theCategory = par.getUntrackedParameter<string>("Category", "Muon|RecoMuon|GlobalMuon|GlobalTruncRefitter");
...
GlobalTruncRefitter::~GlobalTruncRefitter() {
...
// set Event
//
void GlobalTruncRefitter::setEvent(const edm::Event& event) {
  theLayerMeasurements->setEvent(event);  
}



void GlobalTruncRefitter::setServices(const EventSetup& setup){
...
vector<Trajectory> GlobalTruncRefitter::refit(const reco::Track& globalTrack, 
  ...
  LogTrace(theCategory) << " *** GlobalTruncRefitter *** option " << theMuonHitsOption << endl;
  ...
  TrajectoryStateOnSurface currTsos = tsTrans.innerStateOnSurface(globalTrack, *theService->trackingGeometry(), &*theService->magneticField());
  ...
  for (trackingRecHit_iterator hit = track.recHitsBegin(); hit != track.recHitsEnd(); ++hit) {
	...
	allRecHitsTemp.push_back(preciseHit);	
...
//
//
void GlobalTruncRefitter::checkMuonHits(const reco::Track& muon, 
				       ConstRecHitContainer& all,
				       std::vector<int>& hits) const {

  LogTrace(theCategory) << " GlobalTruncRefitter::checkMuonHits " << endl;
...
//
GlobalTruncRefitter::ConstRecHitContainer 
GlobalTruncRefitter::selectMuonHits(const Trajectory& traj, 
...
// print RecHits
//
void GlobalTruncRefitter::printHits(const ConstRecHitContainer& hits) const {
...
//
GlobalTruncRefitter::RefitDirection
GlobalTruncRefitter::checkRecHitsOrdering(const TransientTrackingRecHit::ConstRecHitContainer& recHits) const {
...
//
vector<Trajectory> GlobalTruncRefitter::transform(const reco::Track& newTrack,
...
/*  
  cout << " GlobalTruncRefitter : theFitter " << propDir << endl;
  cout << "                      First TSOS: " 
