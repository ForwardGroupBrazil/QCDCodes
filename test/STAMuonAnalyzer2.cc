...
#include "UserCode/AEverett/test/STAMuonAnalyzer2.h"
...
/// Constructor
STAMuonAnalyzer2::STAMuonAnalyzer2(const ParameterSet& pset){
...
/// Destructor
STAMuonAnalyzer2::~STAMuonAnalyzer2(){
}

void STAMuonAnalyzer2::beginJob(const EventSetup& eventSetup){
...
// my histos...
 hNtracks = new TH1F("hNtracks","N tracks",20,0,20);
...
void STAMuonAnalyzer2::analyze(const Event & event, const EventSetup& eventSetup){
  
  cout << "Run: " << event.id().run() << " Event: " << event.id().event() << endl;
  MuonPatternRecoDumper debug;
  
  // Get the RecTrack collection from the event
  Handle<reco::TrackCollection> simTracks;
  event.getByLabel("standAloneMuons", simTracks);

  Handle<reco::TrackCollection> ctfTrackCol;
  event.getByLabel("ctfWithMaterialTracks", ctfTrackCol);
  const reco::TrackCollection ctfTracks = *(ctfTrackCol.product()); 
  ...
  if(theDataType == "SimData"){
    ...
    reco::TrackCollection::const_iterator simTrack;

// loop on simulated  tracks.....-----------------
    cout<<"Simulated tracks: "<<endl;
    for (simTrack = simTracks->begin(); simTrack != simTracks->end(); ++simTrack){

    reco::TransientTrack track(*simTrack,&*theMGField,theTrackingGeometry); 
      if ( true ) {
	cout<<"Sim pT: "<<track.impactPointTSCP().momentum().perp()<<endl;
	simPt=track.impactPointTSCP().momentum().perp();
        float simEta=track.impactPointTSCP().momentum().eta();

	// clean ghosts...(it will be fixed in 1_0_2 )
	int ighost = 0;
	if (nmusim > 0 ) { 
	  for ( int imu = 1; imu != nmusim+1; ++imu ) {
	    float pT1= sqrt(pxsim[imu]*pxsim[imu] + pysim[imu]*pysim[imu]);
	...
	if ( ighost == 0)  {
 	  nmusim++;
	  pxsim[nmusim] = track.impactPointTSCP().momentum().x();
	  pysim[nmusim] = track.impactPointTSCP().momentum().y();
	  pzsim[nmusim] = track.impactPointTSCP().momentum().z();
          etasim[nmusim] = track.impactPointTSCP().momentum().eta();
	  phisim[nmusim] = track.impactPointTSCP().momentum().phi();
          ptsim[nmusim] = track.impactPointTSCP().momentum().perp();
	  qsim[nmusim] = track.impactPointTSCP().charge(); 
	} 
	//cout<<"Sim Eta: "<<track.impactPointTSCP().momentum().eta()<<endl;
        //cout<<"Sim Phi: "<<track.impactPointTSCP().momentum().phi()<<endl;
	numberOfSimTracks++;
      }    
...
// loop on reco tracks --------------------------
 ...
 cout << " --------- end track ----------------" << endl;    
  ...
  for ( int imu = 1; imu != nmusim+1; ++imu ) {
    ...
    if( ntktracks > 0 ) hetasim-> Fill(etasim[imu]);
    float dRmin = 9999.;
...
DEFINE_FWK_MODULE(STAMuonAnalyzer2)
