void DrawHistos(TString NAME,int REBIN,bool LOG,TString XTITLE)
{
  gROOT->ForceStyle();
  float LUMI(5000);  
  TFile *fDAT = TFile::Open("Histo_flatTree_data_tmva.root");
  TFile *fQCD = TFile::Open("Histo_flatTree_qcd_weights_tmva.root");
  ...
  TCanvas *can = new TCanvas("can_"+NAME,"can_"+NAME,900,600);
  if (LOG)
    gPad->SetLogy(); 
  hQCD->SetMinimum(0.5);
  ...
  TCanvas *canRatio = new TCanvas("can_"+NAME+"Ratio","can_"+NAME+"Ratio",900,600);
  gPad->SetGridy();
  ...
  TCanvas *canStack = new TCanvas("can_Stack"+NAME,"can_Stack"+NAME,900,600);
  if (LOG)
    gPad->SetLogy();
  hQCD->SetMinimum(0.5);
  hQCD->GetYaxis()->SetTitle("Events");
